      ...
      <div class="flex flex-col">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <div v-for="site in item.sites"
               ...
               @click="openNewPage(site.url)">
            <div class="w-full h-full p-3.5 flex items-center">
              <lazy-img
                  class="w-14"
                  :src="'http://www.google.com/s2/favicons?sz=64&domain='+site.url">
              </lazy-img>
              <div class="ml-3.5 h-full">
                <div class="flex flex-col space-start ">
                  <div class="text-indigo-700 ">{{ site.name }}</div>
                  <div class="text-indigo-700 text-xs opacity-60 overflow-hidden overflow-ellipsis">
                    {{ site.desc }}
                  </div>
                </div>
              </div>
...
function searchNavCategories(data: NavCategory[], search: string): NavCategory[] {
  ...
  data.forEach((category) => {
    ...
    if (category.category.toLowerCase().includes(search) || navSites.length > 0) {
      result.push({
